	...
	while (timeout--) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (data != INIT_FL4ECCRESULT_VAL && !checked[i]) {
				...
				if (flctl->page_size)
					index = (512 * sector_number) + (data >> 16);
				else
					index = data >> 16;
...
static void set_cmd_regs(struct mtd_info *mtd, uint32_t cmd, uint32_t flcmcdr_val)
{
	...
	case NAND_CMD_RESET:
		...
		break;
	default:
            	printk( KERN_ERR "%s: unsupported command 0x%x\n", __FUNCTION__, cmd );
		break;
	}
			...
			int column, int page_addr)
{
	...
	case NAND_CMD_RNDOUT:
		...
		empty_fifo(flctl);
		if (flctl->page_size)
			set_cmd_regs(mtd, command, (((command == NAND_CMD_RNDOUT) ? NAND_CMD_RNDOUTSTART : NAND_CMD_READSTART) << 8)
				| command);
		else
...
static int __devinit flctl_probe(struct platform_device *pdev)
{
	...
	flctl_mtd = &flctl->mtd;
        flctl_mtd->name = pdev->name; /* for cmdline partition parsing. default
                                       * name is nand type and has whitespaces */
	nand = &flctl->chip;
	flctl_mtd->priv = nand;
	flctl->pdev = pdev;
	...
	if (ret)
        ...
        /* SWECC would allow SUBPAGE writes, but hwecc doesn't.
           UBIFS images would have to be created differently (-s512 or -s2048),
           therefore we stick with the compatible solution */
	nand->options |= NAND_NO_AUTOINCR | NAND_NO_SUBPAGE_WRITE;
        nand->options   |= NAND_NO_SUBPAGE_READ;

        if (!flctl->hwecc)
                printk( KERN_INFO "Using SW ECC\n" );

        if (pdata->has_bbt) {
                nand->options |= NAND_USE_FLASH_BBT | NAND_BBT_LASTBLOCK;
                printk( KERN_INFO "Using NAND BBT\n" );
        }
	...
	ret = parse_mtd_partitions(flctl_mtd, part_types, &pdata->parts, 0);
	if (ret > 0)
                add_mtd_partitions(flctl_mtd, pdata->parts, ret);
