...
static int ltc2440_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct ltc2440_priv *priv = filp->private_data;
        int err = 0;
	u32 tmp;
...
static int __devinit ltc2440_probe(struct spi_device *spi)
{
...
static int __devexit ltc2440_remove(struct spi_device *spi)
{
...
static struct spi_driver ltc2440_driver = {
	...
	.probe =	ltc2440_probe,
	.remove =	__devexit_p(ltc2440_remove),
...
static int __init ltc2440_init(void)
{
        int   err;

	err = spi_register_driver(&ltc2440_driver);
	if (err < 0)
		return err;
...
static void __exit ltc2440_exit(void)
{
	spi_unregister_driver(&ltc2440_driver);
}

module_init(ltc2440_init);
module_exit(ltc2440_exit);
MODULE_LICENSE("GPL");
