...
static u8 sdhi_clock_leq(unsigned long clk)
{
        ...
        BUG_ON(IS_ERR(bclk));
        bclk_rate = clk_get_rate(bclk);
        clk_put(bclk);

        // find next slowest or matching clock
        while (div <= 512) {
                if (bclk_rate/div <= clk) {
                        u8 reg;

                        switch(div) {
/* Techn. Note. TN-SH7-B023A/E disallows bclk=sd_clk
                            case 1:  reg=0xff;  break;
*/
                            case 2:  reg=0x0;   break;
                            default: reg=div/4; break;
                        }
	...
	/* minimum SD clock possible is bclk/512
           see R8A77230_SDHostInterfacewithoutC2_V03R04E, 1.2.10 */
        pr_err("%s: SD clock not possible %lu", __FUNCTION__, clk);
	/* don't damage hardware */
        BUG();

        return 0x80; /* slowest clock with BCLK/512 */
}
...
static void sdhi_clock_control(int ch, unsigned long clk)
{
	...
	case CLKDEV_MMC_DATA:
		...
		break;
	default:
            	pwrite(ch, SDHI_R18, sdhi_clock_leq(clk));
		break;
	}
	pwrite(ch, SDHI_R18, R18_ENABLE | pread(ch, SDHI_R18));
}
...
static int sdhi_multi_read(int ch, struct mmc_request *mrq)
{
	struct mmc_data *data = mrq->data;
	long time, timeout;
	unsigned short i, sec, j, blocksize;
				...
				struct mmc_request *mrq, unsigned short opc)
{
	...
	case R00_SD_IO_RW_EXTENDED_SREAD:
		...
		break;
	default:
		pr_err(DRIVER_NAME": SD: NOT SUPPORT CMD = d'%04d\n", opc);
		ret = -EINVAL;
		break;
	}
	return ret;
...
static int sdhi_card_inserted(struct mmc_host *mmc)
{
         int i;
         for (i=0; i < mmc->card->sdio_funcs; i++) {
                 struct sdio_func *func = mmc->card->sdio_func[i];

                 if (func->multi_block && func->cur_blksize != 512) {
                         /* only 512 byte are supported */
                         pr_info(DRIVER_NAME ": Disabling multi-block mode for function %i\n", i);
                         func->multi_block = 0;
                 }
         }
...
static struct mmc_host_ops sdhi_ops = {
	...
	.enable_sdio_irq = sdhi_enable_sdio_irq,
        .card_inserted   = sdhi_card_inserted,
};
...
static irqreturn_t sdhi_sdio_intr(int irq, void *dev_id)
{
	...
	struct sdhi_host *host = dev_id;
        ...
        if (host->mmc->sdio_irq_thread == NULL) {
                /* Bogus SDIO hardware might spam us with interrupts,
                 * e.g. CSR1050 after the CPU is resetted while CSR1050 is
                 * used */
                if (printk_ratelimit())
                        pr_err(DRIVER_NAME ": Bogus SDIO interrupt on channel %i, don't handling it\n", host->ch);
                return IRQ_NONE;
        }
...
static int __devinit sdhi_probe(struct platform_device *pdev)
{
	...
	if (pdev->id == 0) {
                ...
                if (!pdev->dev.platform_data) {
                        ...
                        fw_code = (void *)fw_p->data;
                } else
                        fw_code = pdev->dev.platform_data;

		get_address = fw_code;
		get_address(pfunc, fw_code);
		pwrite	= (void *)pfunc[0];
		pread	= (void *)pfunc[1];
	}
	if (firm_check == 1)
	...
	host->ch = pdev->id;
        ...
        switch (host->ch) {
            case 0: module_clk_name = "sdhi0"; break;
            case 1: module_clk_name = "sdhi1"; break;
            default: module_clk_name = NULL; break;
        }

        if (module_clk_name != NULL) {
                host->module_clk = clk_get(NULL, module_clk_name);
                if (host->module_clk == NULL) {
                        pr_err(DRIVER_NAME": clock %s not available\n", module_clk_name);
                        mmc_free_host(mmc);
                        return -ENODEV;
                }

                clk_enable(host->module_clk);
        }
	...
	pr_info("Added SH_SDHI channel %u\n", host->ch);
