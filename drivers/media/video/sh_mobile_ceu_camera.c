...
static int sh_mobile_ceu_capture(struct sh_mobile_ceu_dev *pcdev)
{
	...
	case V4L2_PIX_FMT_GREY:
            /* enable firewall as grey uses data enable fetch mode. External
             * hardware decides how many data to send */
            ceu_write(pcdev, CFWCR, ((phys_addr_top+icd->user_width*icd->user_height) & ~0x1F) | 1);
            break;
				       ...
				       __u32 pixfmt)
{
	...
	switch (icd->current_fmt->host_fmt->fourcc) {
	case V4L2_PIX_FMT_GREY:
            value = 0x0000020;  /* data enable fetch mode */
            break;
	case V4L2_PIX_FMT_NV12:
	case V4L2_PIX_FMT_NV21:
