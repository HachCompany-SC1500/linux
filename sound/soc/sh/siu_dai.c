...
static int siu_dai_start(struct siu_port *port_info)
{
...
//	siu_write32(base + SIU_CKCTL, 0x40400000);
	siu_write32(base + SIU_CKCTL, 0xc0c00000);
			   ...
			   unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBM_CFM:
	    siu_write32(base + SIU_SFORM, 0x00000000);
	    break;
 ...
 * capture, however, the current API sets the bus format globally for a DAI.
 */
static int siu_dai_set_clkdiv(struct snd_soc_dai *dai,
			   int div_id, int div )
{
	struct siu_info *info = siu_i2s_dai.private_data;
	u32 __iomem *base = info->reg;
	...
	switch (div_id) {
		case SIU_CLKA_EXT:
			/* portA's BRG does not divide SIUCKA */
			siu_write32(base + SIU_BRGASEL, 1);
			siu_write32(base + SIU_BRRA, div);
			break;
		case SIU_CLKB_EXT:
			/* portB's BRG divides SIUCKB by half */
			siu_write32(base + SIU_BRGBSEL, 1);
			siu_write32(base + SIU_BRRB, div);
			break;
		default:
			return -EINVAL;
	}
