...
SOC_SINGLE("LCRM Switch", WM8750_ADCTL2, 2, 1, 0),
...
static int wm8750_set_dai_clkdiv(struct snd_soc_dai *codec_dai, int div_id, int div)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	u16 srate = snd_soc_read(codec, WM8750_SRATE);

	switch (div_id) {
	    case WM8750_CLKDIV_ID_BCM:
		srate &= ~(0x3<<7);
		srate |= (div&0x3)<<7;
		break;
	    case WM8750_CLKDIV_ID_MCLK:
		srate &= ~(0x1<<6);
		srate |= (div&0x1)<<6;
		break;
	    default:
...
static struct snd_soc_dai_ops wm8750_dai_ops = {
	...
	.set_clkdiv	= wm8750_set_dai_clkdiv,
};
