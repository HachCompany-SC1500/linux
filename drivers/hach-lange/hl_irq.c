...
static ssize_t hl_irq_read (struct file *file,
                            ...
                            size_t count,
                            loff_t *offset);
static int hl_irq_ioctl (struct inode *inode, struct file *file,
...
static irqreturn_t hl_irq_cci_handler(int irq, void* ptr)
{
  ...
  cTmp  = ctrl_inb(INTREQ00);
  cTmp &= ~0x20;
  ctrl_outb(cTmp, INTREQ00);
...
static int hl_irq_open (struct inode *inode, struct file *file)
{
  ...
  // set PORT_PSELD to IRQ2 fct (bit 10/11: 00)
  sTmp  = ctrl_inw(PORT_PSELD);
  sTmp &= ~0xC00;
  ctrl_outw(sTmp,PORT_PSELD);
  ...
  /* printk("IRQ2: PWCR=%x, PORT_PSELD=%x, HIZCRD=%x\n", */
  /*        (ctrl_inw(PWCR)>>4)&0x3, (ctrl_inw(PORT_PSELD)>>10)&0x3, */
  /*        (ctrl_inw(HIZCRD)>>10)&0x1); */
  /* printk("IRQ2: ICR1=%x, INTPRI00=%x, INTREQ00=%x, INTMSK00=%x\n", */
  /*        (ctrl_inw(ICR1)>>10)&0x3, (ctrl_inl(INTPRI00)>>20)&0xf,  */
  /*        (ctrl_inb(INTREQ00)>>5)&0x1, (ctrl_inb(INTMSK00)>>5)&0x1); */
