...
#define DBG_URB(urb) printk("%s: URB(%p).%s\n",__FUNCTION__,urb,        \
                            urb?urb->dev?urb->dev->product?urb->dev->product:"product=0":"dev=0":"urb=0");
#define DBG_HEP(hep)                                                    \
  { printk("%s: HEP(%p).%s PIP(%p)\n",__FUNCTION__, hep,                \
           hep?(hep->ep_dev?                                            \
                (hep->ep_dev->udev?                                     \
                 (hep->ep_dev->udev->product?hep->ep_dev->udev->product: \
                  "product=0"):                                         \
                 "udev=0"):                                             \
                "ep_dev=0"):                                            \
           "hep=0",hep?hep->hcpriv:0);                                  \
  }

struct pip_elem {
  struct r8a66597_pipe *pipe;
  struct pip_elem      *next;
};
...
static char             PipCnt[R8A66597_MAX_NUM_PIPE];
static struct pip_elem *PipList[R8A66597_MAX_NUM_PIPE];

static void packet_write(struct r8a66597 *r8a66597, u16 pipenum);
static int  r8a66597_get_frame(struct usb_hcd *hcd);

static int  AddPipe(struct r8a66597_pipe *pipe, int pipenum)
{
  struct pip_elem *pipElem;
  if (PipList[pipenum])
  {
    pipElem = PipList[pipenum];
    while (pipElem->next)
      pipElem = pipElem->next;
    pipElem->next = kzalloc(sizeof(struct pip_elem),GFP_ATOMIC);
    if (!pipElem->next)
      return 0; // ERR
    pipElem = pipElem->next;
  }
  ...
  {
    PipList[pipenum] = kzalloc(sizeof(struct pip_elem),GFP_ATOMIC);
    if (!PipList[pipenum])
      return 0; // ERR
    pipElem = PipList[pipenum];
  }
...
static struct r8a66597_pipe *RmPipe(struct r8a66597_pipe *pipe, int pipenum)
{
  struct pip_elem *pipElem;
  struct pip_elem *pipElemPrev;
  
  if (PipList[pipenum])
  {
    pipElem = PipList[pipenum];
    if (PipList[pipenum]->pipe == pipe)
    {
      PipList[pipenum] = PipList[pipenum]->next;
      kfree(pipElem);
    }
    else
    {
      while (pipElem->next)
      {
        pipElemPrev = pipElem;
        pipElem     = pipElem->next;
        if (pipElem->pipe == pipe)
        {
          pipElemPrev->next = pipElem->next;
          kfree(pipElem);
        }
      }
    }
...
static int enable_controller(struct r8a66597 *r8a66597)
{
  ...
  memset(PipCnt ,0,R8A66597_MAX_NUM_PIPE);
  memset(PipList,0,R8A66597_MAX_NUM_PIPE*sizeof(void*));
  return 0;
}
...
{
  // MS: 4 -> no hub, 6 -> 1 hub, 8 -> 2 hubs
  return ((strlen(devpath) >= 8) ? 1 : 0);
}
  ...
  __releases(r8a66597->lock) __acquires(r8a66597->lock)
{
  ...
  if (restart) {
    ...
    if (unlikely(!td))
      return;
    // printk("%s: start_transfer: %s\n",__FUNCTION__, td->urb->dev->product);
				...
				gfp_t mem_flags)
{
  ...
  int ret, request = 0;
  unsigned long flags;
  unsigned char newPipeFlag = false;
  ...
  if (newPipeFlag && td->pipenum > 0) // only handle pipenums > 0
  {
    PipCnt[td->pipenum]++;
    AddPipe(hep->hcpriv,td->pipenum);
    // printk("%s: ADD pipe %d (cnt=%d)\n",__FUNCTION__,td->pipenum,PipCnt[td->pipenum]);
  }
  if (list_empty(&r8a66597->pipe_queue[td->pipenum]))
    request = 1;
				      ...
				      struct usb_host_endpoint *hep)
{
  ...
  struct r8a66597_pipe *replacement_pipe;
  struct r8a66597_td   *td;
  struct urb           *urb = NULL;
  u16                   pipenum;
  unsigned long         flags;
  ...
  spin_lock_irqsave(&r8a66597->lock, flags);
  PipCnt[pipenum] = (PipCnt[pipenum])>0?PipCnt[pipenum]-1:0;
  replacement_pipe = RmPipe(pipe,pipenum);
  pipe_stop(r8a66597, pipe);
  
  if (PipCnt[pipenum] <= 0)
  {
    // printk("%s: STOP pipenum %d (cnt=%d)\n",__FUNCTION__,pipenum,PipCnt[pipenum]);    
    pipe_irq_disable(r8a66597, pipenum);
    disable_irq_empty(r8a66597, pipenum);
  }
  else
  {
    // printk("%s: DON'T stop pipenum %d (cnt=%d)\n",__FUNCTION__,pipenum,PipCnt[pipenum]);
    if (replacement_pipe)
      pipe_start(r8a66597,replacement_pipe);
  }
  td = r8a66597_get_td(r8a66597, pipenum);
  if (td)
    urb = td->urb;
  
  if (urb && urb->dev && urb->dev->product &&
      hep && hep->ep_dev && hep->ep_dev->udev && hep->ep_dev->udev->product &&
      strcmp(urb->dev->product, hep->ep_dev->udev->product))
  {
    /* printk("%s: MISMATCH urb=%s <> hep=%s\n",__FUNCTION__,
       urb->dev->product,hep->ep_dev->udev->product); */
  }
  else
